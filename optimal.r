...
gat_cost <- function(S, T_=10000, c=0, p=8, theta=3, h=1, m=3, gamma=1, type='poi', pois=5, case='FIFO', Bino = FALSE){
  x_t = rep(0, m)
  C_all = 0
  for(t in 1:T_){
    x_t[m] = S
...
test <- function(p_vals, theta_vals, gamma_vals, c_vals, type, PP, h=1, T_=10000,m=3, L=0, case='FIFO', CASE='Lost_sales'){
  ...
  write.csv(headers, paste0("results_", type, "_", case,"_", CASE, "_", start_time, ".csv"), row.names = FALSE)
